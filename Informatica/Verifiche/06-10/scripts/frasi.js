...
genera.addEventListener('click', () => {
    ...
    if (soggettiInput.length === 0 || verbiInput.length === 0 || oggettiInput.length === 0) {
        alert('Inserisci almeno un soggetto, un verbo e un complemento oggetto.');
        return;
    }

    const randomSoggetto = soggettiInput[Math.floor(Math.random() * soggettiInput.length)];
    const randomVerbo = verbiInput[Math.floor(Math.random() * verbiInput.length)];
    const randomOggetto = oggettiInput[Math.floor(Math.random() * oggettiInput.length)];
